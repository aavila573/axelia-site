...
function portada(pres) {
  ...
  s.addText("Automatización con IA para tu Yate y Motos", { x: 0.8, y: 2.6, w: 8.4, h: 1.1, fontSize: 26, bold: true, color: WHITE, align: "center" });
...
function problema(pres) {
  ...
  const problems = [
    "Estás navegando el yate y no respondes → se pierde la salida ($1.400.000)",
    "Brasileños que escriben en portugués, de noche",
    "Las mismas 50 preguntas de las motos, todo el día",
    "No-shows: reservan el yate y no llegan (cupo vacío)",
    "Cobros de anticipos a mano, uno por uno",
  ];
...
function solucionCore(pres) {
  const s = baseSlide(pres, "La solución: tu agente de WhatsApp", "Responde por ti, aunque estés en el agua");
  const items = [
    "Responde 24/7 en español, inglés y portugués",
    "Informa precio y disponibilidad del yate y las motos",
...
function solucionCompleta(pres) {
  ...
  const mods = [
    ["Módulo 1 · Atención & Ventas", "WhatsApp 24/7 trilingüe para yate y motos: reserva y cobra", PURPLE],
    ["Módulo 2 · SEO + GEO", "Te encuentra en Google y en la IA cuando buscan yate o motos", PURPLE2],
    ["Módulo 3 · Video & Redes", "Clips y reels para TikTok/IG, listos para subir", GREEN],
  ];
  mods.forEach((a, i) => {
    const y = 2.25 + i * 0.85;
...
function seoQueEs(pres) {
  ...
  s.addText([
    ...
    { text: "Hoy la mayoría busca «alquiler de yate San Andrés» o «renta de motos San Andrés» en Google antes de escribir. Si no apareces, ese cliente nunca te escribe.", options: { fontSize: 16, color: GRAY, breakLine: true, paraSpaceAfter: 10 } },
...
function videoRedes(pres) {
  ...
  s.addText([
    { text: "Te producimos clips y reels del yate y las motos, listos para subir.", options: { fontSize: 16, color: WHITE, breakLine: true, paraSpaceAfter: 10 } },
...
function flujo(pres) {
  const s = baseSlide(pres, "Cómo funciona", "De la búsqueda a la reserva, sin que sueltes el timón");
  const flow = [
    "Turista busca «yate San Andrés» o ve tu video en TikTok",
    "↓  Escribe al WhatsApp (español, inglés o portugués)",
    "↓  El agente responde en segundos con precio y disponibilidad",
...
function retorno(pres) {
  const s = baseSlide(pres, "El retorno", "Se paga solo con tu yate");
  const roi = [
    "Tu yate se alquila a $1.400.000 por salida",
    "4 salidas de yate = $5.600.000 → pagan todo el sistema",
    "1 salida al mes cubre 4 meses de mantenimiento",
    "Cada salida que no se te escapa es ganancia",
  ];
...
function inversionCompleta(pres) {
  ...
  s.addText("Implementación: 14–21 días · Primera semana para mapear tu yate, tus motos y tus preguntas frecuentes", { x: 0.7, y: 4.85, w: 8.6, h: 0.4, fontSize: 11, color: GRAY, align: "center" });
}
...
function cta(pres) {
  ...
  s.addText("Montamos el agente de WhatsApp con tu yate y tus motos, y ves cómo responde en vivo, sin pagar nada.", { x: 0.8, y: 3.0, w: 8.4, h: 0.9, fontSize: 15, color: GRAY, align: "center" });
...
inversionCore(core);
cta(core);
core.writeFile({ fileName: "C:/Users/alexa/AxelIA_Presentacion_YateMotos_Core.pptx" }).then(() => {
  console.log("✅ Core: AxelIA_Presentacion_YateMotos_Core.pptx");
});
...
cta(completa);
completa.writeFile({ fileName: "C:/Users/alexa/AxelIA_Presentacion_YateMotos_Completa.pptx" }).then(() => {
  console.log("✅ Completa: AxelIA_Presentacion_YateMotos_Completa.pptx");
});
